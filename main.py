...
from exports import export_tasks_to_excel, export_tasks_to_csv, export_tasks_to_gsheet
...
class TaskListWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        self.task_list_sidebar = TaskListSidebar()
...
class TaskListSidebar(QListWidget):
    def __init__(self, parent=None):
        ...
        self.itemClicked.connect(self.load_tasks)
    ...
    def load_tasks(self, item):
        ...
        window.task_table.setRowCount(0)

        # Fetch tasks for the selected task list
        tasks_response = (
            window.service.tasks()
        ...
        window.task_table.horizontalHeaderItem(0).setIcon(QIcon(self.icon_path))

        # Iterate through the tasks and add them to the table
        for task in tasks:
            row_position = window.task_table.rowCount()
            window.task_table.insertRow(row_position)
            window.task_table.setItem(row_position, 0, QTableWidgetItem(task["title"]))
            ...
            if "due" in task:
                ...
                window.task_table.setItem(row_position, 2, QTableWidgetItem(""))
            # Store the task's notes
            task_notes = task.get("notes")
            # Add a button to view more notes
            if task_notes:
                notes_button = QPushButton("View More")
                # clicked passes a checked argument. Have to do notes=task_notes because the lambda gets the value
                # later on. It will get the value from the last iteration if we leave it with task_notes.
                notes_button.clicked.connect(
                    lambda checked, notes=task_notes: window.show_notes(notes)
                )
                window.task_table.setCellWidget(row_position, 3, notes_button)
            # Add priority column
            priority_item = QTableWidgetItem(task.get("priority", "None"))
            window.task_table.setItem(row_position, 4, priority_item)
            # Store the task ID as data for the item
            item = window.task_table.item(row_position, 0)
